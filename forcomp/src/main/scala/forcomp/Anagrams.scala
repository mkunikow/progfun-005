...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(_.toLower).map{case (v, k) => (v, k.length)}.toList.sorted
...
//    dictionary.map(w => (wordOccurrences(w), w)).groupBy(_._1).map{case (k,v) => (k,v.unzip._2)}
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for ((c, i) <- x; i2 = i - ymap(c); if i2 > 0) yield (c, i2)
  }
    ...
    def generateAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.length == 0 ) List(List())
        ...
        for {
          ...
          sentence <- generateAnagrams(subtract(occurrences, o))
        } yield w :: sentence
      }
    }

    val occur = sentenceOccurrences(sentence)
    generateAnagrams(occur)
  }
